...
class UpcomingMoviesAdapter(
    ...
    private val upcoming: ArrayList<UpcomingMoviesModel.Result>,
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        try {
            ...
            val imgUrl = baseImageURL + upcoming.get(position).posterPath
            Picasso.with(contex)
                .load(imgUrl)
            ...
            holder.title.text = upcoming.get(position).title
        } catch (e: Exception) {
            e.printStackTrace()
        }

        holder.itemView.setOnClickListener(View.OnClickListener {
            onItemCLick.onItemClick(position, upcomingMoviee)
